...
sys.path.extend([".", ".."])
from transm import albert
from CONSTANTS import *
from sklearn.decomposition import FastICA
from representations.templates.statistics_T import Simple_template_TF_IDF, Template_TF_IDF_without_clean
...
if __name__ == '__main__':
    mm = albert()
    res = mm.return_emmbeding("Hello this is test for running transformer")
...
###########    
    ...
    with open(filename, 'wb') as fi:
        # dump your data into the file
      pickle.dump(dev, fi)
    filename = 'processor_T.pk'
    with open(filename, 'wb') as fi:
        # dump your data into the file
      pickle.dump(processor, fi)
    filename = 'num_classes_T.pk'
    with open(filename, 'wb') as fi:
        # dump your data into the file
      pickle.dump(num_classes, fi)
###################
    with open("test_T.pk", 'rb') as fi:
      test = pickle.load(fi)
    with open('train_T.pk', 'rb') as fi:
      train = pickle.load(fi)
    with open('dev_T.pk', 'rb') as fi:
      dev = pickle.load(fi)
    with open('processor_T.pk', 'rb') as fi:
      processor = pickle.load(fi)
    with open('num_classes_T.pk', 'rb') as fi:
      num_classes = pickle.load(fi)
